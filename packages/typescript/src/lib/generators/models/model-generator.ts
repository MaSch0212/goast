  ...
  implements TypeScriptModelGenerator
{
  ...
  protected getInterface(ctx: Context, schema: ObjectLikeApiSchema) {
    return tsInterface(this.getDeclarationTypeName(ctx, ctx.schema), {
      export: true,
  ...
  protected getArrayType(ctx: Context, schema: ArrayLikeApiSchema): AppendValue<Builder> {
    return tsArrayType(this.getType(ctx, schema.items), { readonly: ctx.config.immutableTypes });
  }
  ...
  protected getOneOfType(ctx: Context, schema: ApiSchema<'oneOf'>): AppendValue<Builder> {
    if (schema.oneOf.length === 0) {
      return this.getAnyType(ctx);
    }
    return tsUnionType(schema.oneOf.map((x) => this.getType(ctx, x)));
  }

  protected getMultiType(ctx: Context, schema: ApiSchema<'multi-type'>): AppendValue<Builder> {

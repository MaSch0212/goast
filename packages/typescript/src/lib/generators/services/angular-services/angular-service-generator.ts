...
import { TypeScriptExternalTypeOptions, TypeScriptFileBuilder } from '../../../file-builder';
  ...
  implements TypeScriptAngularServiceGenerator
{
  ...
  protected getServiceFileContent(ctx: Context): TsCode {
    const code = tsCode();
    for (const endpoint of ctx.service.endpoints) {
      if (endpoint.parameters.length > 0 || endpoint.requestBody !== undefined) {
        code.parts.push(this.getEndpointParamsType(ctx, endpoint));
      }
    }
    code.parts.push(this.getClass(ctx));
    return code;
  }

  protected getEndpointParamsType(ctx: Context, endpoint: ApiEndpoint) {
    const type = tsObjectType();

    for (const parameter of endpoint.parameters) {
      const schema = parameter.schema;
      type.properties.push(
        tsProperty(parameter.name, {
    ...
    return tsTypeAlias(this.getEndpointParamsTypeName(ctx, endpoint), {
      doc: tsDoc({ description: `Parameters for operation ${this.getEndpointMethodName(ctx, endpoint)}` }),
      type,
    });
  }

  protected getClass(ctx: Context) {
    return tsClass(this.getServiceClassName(ctx), {
      doc: tsDoc({ description: ctx.service.description }),
      decorators: [tsDecorator(tsReference('Injectable', '@angular/core'), [toTsObject({ providedIn: 'root' })])],
      extends: tsReference('BaseService', resolve(this.getUtilsDirPath(ctx), 'base-service.ts')),
      // TODO
    });
  }

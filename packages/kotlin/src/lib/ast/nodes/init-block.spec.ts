...
describe('ktInitBlock', () => {
  ...
  it('should write an init block with a single statement', () => {
    ...
    expect(builder.toString(false)).toBe(`init {${EOL}    println("Hello")${EOL}}`);
  });
  ...
  it('should write an init block with multiple statements', () => {
    ...
    expect(builder.toString(false)).toBe(`init {${EOL}    println("Hello")${EOL}    println("World")${EOL}}`);
  });

  it('should render injections', () => {
    builder.append(ktInitBlock('println("Hello")', { inject: { before: ['before'], after: ['after'] } }));
    expect(builder.toString(false)).toBe(`beforeinit {${EOL}    println("Hello")${EOL}}after`);
  });
});

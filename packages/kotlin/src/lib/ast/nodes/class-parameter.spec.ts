...
import { ktClassParameter, writeKtClassParameters } from './class-parameter';
...
describe('multiple', () => {
  it('should write parenthesis if the node is empty', () => {
    writeKtClassParameters(builder, []);
    expect(builder.toString(false)).toBe('()');
  });

  it('should write a single parameter', () => {
    writeKtClassParameters(builder, [ktClassParameter('x', 'Int')]);
    expect(builder.toString(false)).toBe(`(x: Int)`);
  });

  it('should write all the parameters', () => {
    writeKtClassParameters(builder, [ktClassParameter('x', 'Int'), ktClassParameter('y', 'String')]);
  ...
  it('should multiline if there are more than 2 parameters', () => {
    writeKtClassParameters(builder, [
      ktClassParameter('x', 'Int'),
      ktClassParameter('y', 'String'),
      ktClassParameter('z', 'Boolean'),
    ]);
    expect(builder.toString(false)).toBe(`(${EOL}    x: Int,${EOL}    y: String,${EOL}    z: Boolean${EOL})`);
  });

  it('should multiline if there are annotations', () => {
    writeKtClassParameters(builder, [ktClassParameter('x', 'Int', { annotations: [ktAnnotation('Inject')] })]);

...
export class DefaultKotlinSpringReactiveWebClientGenerator extends KotlinFileGenerator<Context, Output>
  ...
  protected getEndpointMembers(ctx: Context, args: Args.GetEndpointMembers): kt.ObjectMember<Builder>[] {
    ...
    return [
      ...
      this.getEndpointHttpInfoFunction(ctx, { endpoint, parameters, responseSchema }),
  ...
  protected getEndpointFunctionBody(ctx: Context, args: Args.GetEndpointFunctionBody): AppendValueGroup<Builder> {
    ...
    const requestCall = kt.call(['this', requestFunctionName], parameterNames);

    if (responseSchema) {
      const awaitBodyCall = kt.call([
        requestCall,
        kt.refs.springReactive.awaitBody([
          this.getTypeUsage(ctx, { schema: responseSchema, fallback: kt.refs.unit() }),
        ]),
      ], []);
      result.values.push(s`return ${awaitBodyCall}`);
    } else {
      result.values.push(kt.call([requestCall, kt.refs.springReactive.awaitBodilessEntity()], []));
    }

    return result;
  }

  protected getEndpointHttpInfoFunctionName(ctx: Context, args: Args.GetEndpointHttpInfoFunctionName): string {
    const { endpoint } = args;
    return toCasing(`${this.getEndpointFunctionName(ctx, { endpoint })}_httpInfo`, ctx.config.functionNameCasing);
  }

  protected getEndpointHttpInfoFunction(ctx: Context, args: Args.GetEndpointHttpInfoFunction): kt.Function<Builder> {
    const { endpoint, parameters, responseSchema } = args;
    const functionName = this.getEndpointHttpInfoFunctionName(ctx, { endpoint });
    ...
    return kt.function(functionName, {
      ...
      returnType: kt.refs.spring.responseEntity([
        this.getTypeUsage(ctx, { schema: responseSchema, fallback: kt.refs.java.void_() }),
      ]),
      body: this.getEndpointHttpInfoFunctionBody(ctx, { endpoint, parameters, responseSchema }),
    });
  }

  protected getEndpointHttpInfoFunctionBody(
    ctx: Context,
    args: Args.GetEndpointHttpInfoFunctionBody,
    ...
    if (responseSchema) {
      const toEntityCall = kt.call([
        requestCall,
        kt.refs.springReactive.toEntity([
          this.getTypeUsage(ctx, { schema: responseSchema, fallback: kt.refs.unit() }),
        ]),
      ], []);
      result.values.push(s`return ${kt.call([toEntityCall, kt.refs.kotlinx.awaitFirst()], [])}`);
    } else {
      result.values.push(s`return ${kt.call([requestCall, kt.refs.springReactive.awaitBodilessEntity()], [])}`);
    }
  ...
  protected getEndpointRequestFunction(ctx: Context, args: Args.GetEndpointRequestFunction): kt.Function<Builder> {
    ...
    return kt.function(functionName, {
      ...
      returnType: kt.refs.springReactive.ResponseSpec(),
  ...
  protected getEndpointRequestFunctionBody(
    ...
    callChain.push(
      ...
      kt.call('uri', [this.getEndpointUri(ctx, { endpoint })]),
    );
    ...
    if (endpoint.requestBody) {
      if (endpoint.requestBody.content[0]?.type === 'multipart/form-data') {
                    ...
                    appendValueGroup(
                      parameters.filter((x) => x.multipart).map((p) => {
                        ...
                        return kt.call([
                          ...
                          p.multipart!.isFile ? kt.call('filename', [`${parameterName}.name`]) : null,
                          kt.call('contentType', [
                            kt.call([
                              kt.refs.spring.mediaType(),
                              p.multipart!.isFile ? 'APPLICATION_OCTET_STREAM' : 'APPLICATION_JSON',
                            ]),
                          ]),
                        ]);
                      }),
        ...
        callChain.push(
          kt.call('bodyValue', [
            toCasing(this.getRequestBodyParamName(ctx, { endpoint }), ctx.config.parameterNameCasing),
          ]),
        );
      }
    }
          ...
          kt.lambda(
            ...
            appendValueGroup(
              parameters.filter((x) => x.target === 'header').map((p) =>
                s<Builder>`it.add(${kt.string(p.name)}, ${toCasing(p.name, ctx.config.parameterNameCasing)}${
    ...
    callChain.push('retrieve');
    result.values.push(kt.call(callChain, []));
    return result;
  }

  protected getEndpointUri(ctx: Context, args: Args.GetPathWithInterpolation): kt.Value<Builder> {
    const { endpoint } = args;
          ...
          appendValueGroup(
            endpoint.parameters.filter((x) => x.target === 'query').map((p) => {
              ...
              return p.required && !p.schema?.nullable
                ...
                : kt.call([`${parameterName}?`, 'run'], [
                  kt.lambda([], kt.call('queryParam', [kt.string(p.name), parameterName + toString])),
                ]);
            }),
            '\n',
          ),
